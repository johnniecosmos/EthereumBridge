...
def sign_tx(unsigned_tx: str, multi_sig_account_addr: str, account_name: str):
    ...
    cmd = [secret_cli_posix, 'tx', 'sign', unsigned_tx_posix, '--signature-only', '--multisig',multi_sig_account_addr,
           '--from', account_name]
...
def multisign_tx(unsigned_tx_path: str, multi_sig_account_name: str, *signed_tx):
    ...
    return run_secret_cli([secret_cli_posix, 'tx', 'multisign', unsigned_tx_path, multi_sig_account_name]+
...
def create_unsigined_tx(secret_contract_addr: str, encoded_args: str, multisig_acc_addr: str) -> str:
    ...
    cmd = [secret_cli_posix, 'tx', 'compute', 'execute', secret_contract_addr, f"'{encoded_args}'",
...
def broadcast(signed_tx_path: str):
    ...
    cmd = [secret_cli_posix, 'tx', 'broadcast', signed_tx_path_posix]
    return run_secret_cli(cmd)
    # return run_secret_cli(" ".join(cmd))
